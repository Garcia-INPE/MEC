...
for (ltime.num in 1:ltime2) {
  ...
  num.lin <- 0L
  for (f in files) {
    #cat(paste0(DIR$eval, f))
    load(paste0(DIR$eval, f))
    num.lin <- num.lin + nrow(EVAL.CONT.PER)
  }
  cat(num.lin, 'lines, ')
  CONT.UPDATE <- data.table(EV_SET=character(num.lin), MOD=character(num.lin), LTIME=ltime.num, EVAL.CONT.PER[0][, list(REG, MASK, PER, HH, SOBS, SFCT, SBIAS, SCORR, SRMSE)]) 
  ...
  for (f in files) {
    ...
    set(CONT.UPDATE, (num.lin+1):(num.lin+nrow(EVAL.CONT.PER)), 1:ncol(CONT.UPDATE), 
        cbind.data.frame(EV.SET, MOD, ltime.num, EVAL.CONT.PER[, list(REG, MASK, PER=paste0(CFG$val_dt1, "-", CFG$val_dt2), HH=CFG$init_time, SOBS, SFCT, SBIAS, SCORR, SRMSE)]))
    num.lin <- num.lin + nrow(EVAL.CONT.PER)
  }
  ...
  save(EVAL.CONT.PER, file=paste0(DIR$eval, '/EVAL_CONT_PER_', ltime.num, "_", MEC_init_time, '.RData'))
  cat('ok!\n')
}
