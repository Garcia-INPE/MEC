...
if (!file.exists(fname_eval_cont)) {
  ...
  if ("ALL" %in% CFG.SET$masks) {
    comm = paste0("EVAL.CONT.PER <- data.table(expand.grid(REG=CFG$area1.names[1], MASK='ALL', PER='", season, "', HH='", CFG$init_time, "', ", paste0(CFG$sp_metrics, "=NA_real_", collapse=", "), "))")
      eval(parse(text=comm))
  } else {
    EVAL.CONT.PER = data.table()
  }
  # Acrescenta todas as regiões somente com a máscara CONT
  comm = paste0("EVAL.CONT.PER <- rbind.data.frame(EVAL.CONT.PER, data.table(expand.grid(REG=c(CFG$area1.names, REGIONS$NAME), MASK='CONT', PER='", season, "', HH='", CFG$init_time, "', ", paste0(CFG$sp_metrics, "=NA_real_", collapse=", "), ")))")
  eval(parse(text=comm))
...
if (CFG.SET$eval.cat) {
  ...
  if (!file.exists(fname_eval_cat)) {
    # Só a região maior (a primeira) é sem máscara, caso seja para avaliar sem máscara
    if ("ALL" %in% CFG.SET$masks) {
      comm = paste0("EVAL.CAT.PER <- data.table(expand.grid(REG=CFG$area1.names[1], MASK='ALL', THRES=thresholds, PER='", season, "', HH='", CFG$init_time, "', ", paste0(METRICS$CT, "=NA_integer_", collapse=", "), "))")
        eval(parse(text=comm))
    } else {
      EVAL.CAT.PER = data.table()
    }
    # Acrescenta todas as regiões somente com a máscara CONTinente
    comm = paste0("EVAL.CAT.PER <- rbind.data.frame(EVAL.CAT.PER, data.table(expand.grid(REG=c(CFG$area1.names, REGIONS$NAME), MASK='CONT', THRES=thresholds, PER='", season, "', HH='", CFG$init_time, "', ", paste0(METRICS$CT, "=NA_integer_", collapse=", "), ")))")
    eval(parse(text=comm))
